...
export function ActiveFilterPills({ filters, setFilter, onClearAll }: ActiveFilterPillsProps) {
  ...
  if (filters.status) {
    pills.push({
      ...
      colorClass: STATUS_COLORS[filters.status] || 'bg-ctp-mauve/20 border-ctp-mauve/40 text-ctp-mauve',
    })
  }
  ...
  const handleRemove = (pill: FilterPill) => {
    if (pill.key === 'favorites') {
      setFilter(pill.key, false)
    } else if (pill.itemValue && (pill.key === 'genre' || pill.key === 'collection' || pill.key === 'franchise')) {
      const currentArray = filters[pill.key] as string[]
      const newArray = currentArray.filter((item) => item !== pill.itemValue)
      setFilter(pill.key, newArray as any)
    } else {
      setFilter(pill.key, '' as any)
    }
  }
